function [ varargout ] = Sampling_Jittered( image_, nSamples_, displaySamples_ )
%SAMPLING_JITTERED Spatially samples an image using a jittered approach.
%   INPUTS:
...
elseif numel( nSamples_ ) == 2
    ...
    display( 'Sampling_Jittered: nSamples_ must be a vector of 1 or 2 elements' )
    return;
end
...
%% generate the jitter
jitter = [ dCol*rand( [ 1 size( samplePoints, 2 ) ] ) - ( dCol/2 );
            dRow*rand( [ 1 size( samplePoints, 2 ) ] ) - ( dRow/2 ) ];
samplePoints = samplePoints + jitter;
